const copy = {
    en: {
        ...
        cinematicFilm: 'Cinematic film', originalFrames: 'Original frames', interactiveModel: 'Interactive model', enterPrivateRoom: 'Enter the interactive room',
        selectionNote: 'The selection note', price: 'Price', priceNote: 'VAT status and financing are confirmed during the consultation.',
        bookViewing: 'Book a private viewing', bookViewingShort: 'Book viewing', exploreFinance: 'Explore financing',
        editorialReasons: 'Editorial reasons', whyThisOne: 'Why this one.', whySub: 'Three details that define the character of this particular car.',
        visualRecord: 'Visual record', closerLook: 'A closer look.', gallerySub: 'Select a frame to bring the details forward.',
        ...
        technicalArchive: 'Technical archive', everythingWorth: 'Everything worth knowing.', specsSub: 'Organised by use, not by noise.',
    ...
    ro: {
        ...
        cinematicFilm: 'Film cinematic', originalFrames: 'Cadre originale', interactiveModel: 'Model interactiv', enterPrivateRoom: 'Intră în camera interactivă',
        selectionNote: 'Nota selecției', price: 'Preț', priceNote: 'Regimul TVA și opțiunile de finanțare se confirmă în timpul consultanței.',
        bookViewing: 'Programează o vizionare privată', bookViewingShort: 'Programează', exploreFinance: 'Vezi finanțarea',
        editorialReasons: 'Argumente editoriale', whyThisOne: 'De ce acest exemplar.', whySub: 'Trei detalii care definesc personalitatea acestei mașini.',
        visualRecord: 'Jurnal vizual', closerLook: 'Privește mai atent.', gallerySub: 'Alege un cadru și adu detaliile în prim-plan.',
        ...
        technicalArchive: 'Arhivă tehnică', everythingWorth: 'Tot ce merită știut.', specsSub: 'Organizat după utilitate, nu după zgomot.',
...
function getEditorial() { return editorial[state.car.id]?.[state.lang] || genericEditorial(state.car, state.lang); }
...
function renderGallery() {
    state.imageIndex = Math.min(state.imageIndex, state.images.length - 1);
    ...
    $('#gallery-total').textContent = `/ ${String(state.images.length).padStart(2, '0')}`;
    const rail = $('#gallery-rail');
    rail.replaceChildren(...state.images.map((src, index) => {
    ...
    $('#gallery-next').hidden = state.images.length < 2;
}
...
function changeImage(nextIndex) {
    ...
    window.setTimeout(() => {
        state.imageIndex = (nextIndex + state.images.length) % state.images.length;
...
function render() {
    ...
    heroTitle.replaceChildren(element('span', '', state.car.make), element('span', '', state.car.model));
    ...
    $('#final-heading').textContent = edit.final;
    ...
    if (isSignature) {
        ...
        const link = $('#signature-3d-link');
        link.href = state.car.sketchfab || '#dossier-intro';
        link.target = state.car.sketchfab ? '_blank' : '_self';
        // Contoarele erau scrise in markup: 01, 14, 360 grade. Le citim din
        // masina, altfel orice alta masina promovata ar sustine ca are 14 cadre.
        $('#signature-frame-count').textContent = String(state.images.length).padStart(2, '0');
        $('#signature-film-fact').hidden = !state.car.video;
        $('#signature-model-fact').hidden = !state.car.sketchfab;
    }
    renderFacts();
    renderReasons(edit);
    renderGallery();
